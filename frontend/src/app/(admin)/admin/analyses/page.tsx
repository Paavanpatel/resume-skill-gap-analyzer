...
export default function AdminAnalysesPage() {
  ...
  const handleRetry = async (id: string) => {
    ...
    try {
      ...
      toast("Analysis re-queued.", "success");
      fetchAnalyses();
    } catch (e) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        variant={variant}
                        className="text-xs capitalize inline-flex items-center gap-1"
                      >
                        <StatusIcon
                          className={cn("h-3 w-3", a.status === "processing" && "animate-spin")}
                        />
                        {a.status}
                      </Badge>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="min-w-0 max-w-[200px]">
                        ...
                        {a.error_message && (
                          <p
                            className="text-xs text-danger-500 truncate mt-0.5"
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
                      {a.ai_provider ? (
                        <span title={a.ai_model || undefined}>{a.ai_provider}</span>
                      ) : (
                        "—"
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {a.status === "failed" && (
                          ...
                          >
                            <RefreshCw
                              className={cn("h-3.5 w-3.5", retrying === a.id && "animate-spin")}
                            />
                          </button>
                        )}
                        <button
      ...
      {deleteTarget && (
        <Modal isOpen onClose={() => setDeleteTarget(null)} title="Delete Analysis">
