...
| 23 | Full-Stack Phase 8 — File Storage Migration (Local → S3/MinIO) | COMPLETE | Backend: `StorageBackend` Protocol + `LocalStorage` + `S3Storage` (aioboto3), `get_storage()` factory, `upload_bytes()` for raw key writes, `generate_presigned_url()`, `get_stats()`; S3/MinIO config fields (`s3_bucket`, `s3_region`, `s3_access_key_id`, `s3_secret_access_key`, `s3_endpoint_url`, `s3_presigned_url_expiry_seconds`); `GET /insights/{id}/export-url` endpoint (uploads PDF to S3, returns presigned URL; falls back to null for local); `GET /admin/storage/stats` endpoint + `StorageStats` schema; `scripts/migrate_to_s3.py` migration script. Docker: MinIO service + `minio-init` bucket-create service + `minio_data` volume. Frontend: `ExportButton` presigned URL flow (probe for expiry → re-fetch on 403, fallback to blob for local); `getExportUrl` + `adminGetStorageStats` in api.ts; Admin analytics: Storage card (backend, bucket, file count, bytes used). |
| 24 | Full-Stack Phase 9 — Observability, Logging & Health Checks | COMPLETE | Backend: `app/core/metrics.py` — isolated Prometheus `CollectorRegistry` with 6 metric families (http_requests_total, http_request_duration_seconds, analyses_total, analysis_duration_seconds, llm_calls_total, llm_tokens_total); `app/core/log_buffer.py` — thread-safe in-memory circular buffer (500 entries, `_LogBufferHandler`); `logging_config.py` updated to attach log buffer handler + `foreign_pre_chain` for stdlib integration; `RequestIdMiddleware` now binds structlog contextvars (request_id, method, path) per request and records HTTP metrics; `_normalise_path()` collapses UUID/numeric path segments to `{id}` to prevent label-cardinality explosion; `_EXEMPT_PATHS` extended to include `/health/live`, `/health/ready`, `/metrics`; `GET /api/v1/health/live` (liveness probe, no deps); `GET /api/v1/health/ready` (readiness: DB + Redis + Celery in parallel, 503 on unhealthy); `GET /api/v1/metrics` (Prometheus text, admin-only); `GET /api/v1/admin/system/metrics` (JSON summary for admin UI); `GET /api/v1/admin/system/logs` (recent buffer, filterable by level + logger prefix); `prometheus-client==0.21.0` added to requirements.txt; old inline `/api/v1/health` retained as 307 redirect alias. Frontend: `useHealthCheck` hook (30s polling, exposes HealthState); `StatusIndicator` component (pulsing dot + click-to-open popover with per-dep check rows); Navbar health dot (hidden on mobile, shown ≥sm); `getHealthLive`, `getHealthReady`, `adminGetMetricsSummary`, `adminGetLogs` in api.ts; X-Request-ID captured in response interceptor (`api:request-error` CustomEvent + included in `getErrorMessage`); Admin `/system` page (health probe dashboard, 4-card metrics grid, HTTP status-class breakdown, log viewer with level/prefix filters). Admin sidebar updated with System entry. |
| 25 | Full-Stack Phase 10 — CI/CD Pipeline & Production Hardening | COMPLETE | GitHub Actions CI (6-job pipeline: lint-backend ruff, lint-frontend ESLint+Prettier+tsc, test-backend pytest 70% floor, test-frontend Jest 80% threshold, build-docker BuildKit cache, bundle-size 500 kB page budget); GitHub Actions deploy (GHCR image push + SSH rolling deploy + Alembic migration gate + smoke test); Celery beat `backup_task.py` (daily pg_dump at 02:00 UTC, 7-day retention, prune helper) + `weekly_sweep_stale` (Sunday 03:00 UTC); beat service added to `docker-compose.prod.yml`; `postgresql-client` added to `backend/Dockerfile.prod` for pg_dump; Frontend: custom 404 page (`not-found.tsx`), dashboard `loading.tsx` skeleton, `OfflineBanner` component (online/offline events, reconnected flash, mounted in root layout), `/maintenance` static page; `next-sitemap` (config + robots.txt, public-pages-only, per-path priority overrides); `package.json` updated with `prettier@3`, `next-sitemap`, `format`/`format:check`/`test:ci` scripts, `postbuild` sitemap generation; `next.config.js` image optimisation (AVIF+WebP formats, S3 + MinIO remotePatterns, deviceSizes); `.env.example` and `.env.production.example` completed with email, Stripe, S3, beat, sitemap vars; `.prettierrc` + `.prettierignore` added. |

### 2.10 Full-Stack Phase 10 — CI/CD Pipeline & Production Hardening (COMPLETE)

**Backend deliverables:**

- `.github/workflows/ci.yml` — 6-job GitHub Actions pipeline (concurrency group `ci-$ref`, cancel-in-progress). Jobs: `lint-backend` (ruff check + ruff format --check on `app/`), `lint-frontend` (ESLint + prettier --check + tsc --noEmit), `test-backend` (pytest with postgres 16 + redis 7 services, 70% coverage floor, XML report artifact), `test-frontend` (Jest with 80%/80%/70%/80% lines/fns/branches/stmts threshold, coverage artifact), `build-docker` (BuildKit + GHA layer cache for backend + frontend images), `bundle-size` (walks `.next/static/chunks`, fails if any JS chunk >512 kB).
- `.github/workflows/deploy.yml` — Deploy pipeline: triggers on push to `main` or `workflow_dispatch` (environment choice). Jobs: `check-ci` (validates CI checks on the commit), `build-push` (logs in to GHCR, builds + pushes backend + frontend images tagged with short SHA + `latest`; BuildKit GHA cache), `deploy` (SSH via appleboy/ssh-action: pulls images, runs `alembic upgrade head` in a one-off container, rolling restart of backend/worker/beat/frontend, prunes dangling images, smoke test curl on `/api/v1/health/live`). Concurrency lock prevents parallel deploys per environment.
- `backend/app/workers/backup_task.py` — Two Celery tasks: `daily_db_backup` (subprocess `pg_dump --format=custom --compress=6`, writes to `BACKUP_DIR`, prunes to `BACKUP_RETENTION_COUNT` latest dumps, max_retries=2, 5-min retry delay) and `weekly_sweep_stale` (marks analyses stuck in PROCESSING >24 h as FAILED via SQLAlchemy `update(...).returning()`, max_retries=1). Both are async-safe via `asyncio.run()`.
- `backend/app/workers/celery_app.py` — Added `beat_schedule`: `daily-db-backup` at 02:00 UTC (crontab hour=2, minute=0) and `weekly-sweep-stale` at 03:00 UTC Sunday (day_of_week=0). Imported `backup_task` for Celery task registration.
- `backend/Dockerfile.prod` — Added `postgresql-client` apt package to runtime stage so `pg_dump` is available inside the container.
- `docker-compose.prod.yml` — Added `beat` service (Celery beat scheduler, `PersistentScheduler`, `/tmp/celerybeat-schedule`, 256 MB / 0.25 CPU limit, backend-net).

**Frontend deliverables:**

- `src/app/not-found.tsx` — Custom 404 page: logo mark, large "404" number, h1 + description, "Go to Dashboard" + "Back to Home" CTAs. `robots: { index: false }` in metadata.
- `src/app/(dashboard)/loading.tsx` — Route-group loading skeleton for the dashboard. Pulsing `animate-pulse` skeleton matching usage widget, heading, progress steps, and main card layout. Accessible with `aria-busy` + `aria-label`.
- `src/components/ui/OfflineBanner.tsx` — Sticky `z-[9999]` top banner. Uses `navigator.onLine` + `window.addEventListener('offline'/'online')`. Red banner when offline, green flash "Back online!" for 3 s after reconnect. Mounted in root layout so it covers every route.
- `src/app/layout.tsx` — `<OfflineBanner />` added above `<SkipToContent />`.
- `src/app/maintenance/page.tsx` — Static maintenance page with logo, wrench icon, page title, 3-row status table (API: under maintenance, Database: under maintenance, Static assets: operational). No API calls — works without backend. `robots: { index: false }`.
- `frontend/next-sitemap.config.js` — Configured with `siteUrl` from `SITE_URL` env var, `generateRobotsTxt: true`, excludes all authenticated/admin routes, per-path priority overrides (/ → 1.0, /pricing → 0.9, /register → 0.7, /login → 0.6). `postbuild` script in package.json runs `next-sitemap` automatically.
- `frontend/package.json` — Added `next-sitemap@^4.2.3` (dependency) and `prettier@^3.3.3` (devDependency). New scripts: `postbuild` (next-sitemap), `lint:strict` (--max-warnings 0), `format` (prettier --write), `format:check` (prettier --check), `test:ci` (jest --ci with coverage thresholds).
- `frontend/.prettierrc` — Prettier config: semi=true, singleQuote=false, trailingComma=es5, printWidth=100, tabWidth=2.
- `frontend/.prettierignore` — Excludes `.next/`, `node_modules/`, `coverage/`, `public/`, lockfiles.
- `frontend/next.config.js` — Added `images` block: `formats: ['image/avif', 'image/webp']`, `remotePatterns` for AWS S3 (`*.s3.amazonaws.com`) and MinIO (`localhost:9000`), `deviceSizes` and `imageSizes` limits.
- `.env.example` — Extended with email, Stripe, SITE_URL, BACKUP_RETENTION_COUNT, BACKUP_DIR sections.
- `.env.production.example` — Fully completed with all vars from all 25 phases: email SMTP, Stripe keys, S3, beat backup settings, SITE_URL, FRONTEND_URL.
...
- `DEPLOY_PATH` — Absolute path on server (e.g. `/opt/rsga`)
...
- ~~**File storage**: Uploaded resumes are stored on local filesystem. Need migration to S3/MinIO for production.~~ (Done — Phase 8)
...
| 2026-03-28 | Phase 24 (Full-Stack Phase 9 — Observability, Logging & Health Checks) complete. Backend: `prometheus-client==0.21.0`; `app/core/metrics.py` (6 Prometheus metric families in isolated registry); `app/core/log_buffer.py` (thread-safe 500-entry circular buffer + `_LogBufferHandler`); `logging_config.py` updated (buffer handler + `foreign_pre_chain`); `RequestIdMiddleware` now binds structlog contextvars + records HTTP metrics via `_normalise_path`; `RateLimitMiddleware._EXEMPT_PATHS` extended; `GET /health/live` + `GET /health/ready` (DB+Redis+Celery concurrent probes, 503 on unhealthy); `GET /metrics` (Prometheus text, admin-only); `GET /admin/system/metrics` + `GET /admin/system/logs` admin endpoints; `/api/v1/health` kept as 307 alias. Frontend: `useHealthCheck` hook (30s poll); `StatusIndicator` (pulsing dot + click popover); Navbar health dot (≥sm breakpoint); `getHealthLive`/`getHealthReady`/`adminGetMetricsSummary`/`adminGetLogs` + `HealthCheck`/`MetricsSummary`/`LogRecord` types in api.ts; X-Request-ID captured in response interceptor (`api:request-error` event + `getErrorMessage`); Admin `/system` page (health checks, 4-metric cards, status-class breakdown, log viewer with level+prefix filters); admin sidebar System entry. |
